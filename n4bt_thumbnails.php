<?php
/*
Plugin Name: N4BT Thumbnails
Plugin URI:  https://nnnn.blog/n4bt-thumbnails.html
...
N4BT Thumbnails is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 2 of the License, or
any later version.
 
N4BT Thumbnails is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU General Public License for more details.
 
You should have received a copy of the GNU General Public License
along with N4BT Thumbnails. If not, see https://www.gnu.org/licenses/gpl-2.0.html.
*/

define('N4BT_PLUGIN_URL', plugins_url('', __FILE__));
define('N4BT_PLUGIN_DIR', plugin_dir_path(__FILE__));
function n4bt_load_assets(){
	wp_enqueue_style("n4bt_thumbnails",N4BT_PLUGIN_URL.'/css/n4bt.thumbnails.min.css',array());
	wp_enqueue_script("n4bt_thumbnails",N4BT_PLUGIN_URL.'/js/n4bt.thumbnails.min.js',array('jquery'),'',true);
	wp_enqueue_media();
}
add_action('admin_enqueue_scripts', 'n4bt_load_assets');
/*
	给{taxonomy} 添加新的特色图像
*/
class N4BT_ThumbnailsTaxonomy {
	...
	public function add_boxed($taxonomy){
		...
		if(!$this->options['multiple']){
			...
			if(is_object($taxonomy)){
				$value = get_n4bt_thumbnails($taxonomy->term_id,$this->options['name']);
				$attachment = wp_get_attachment_metadata($value);
				$attachment = array_merge((array)$attachment,array('url' => wp_get_attachment_url($value)));
			}
		?>
		<div id="n4bt-thumbnails" class="n4bt_maxw" data-options='<?php echo json_encode($this->options); ?>' <?php if($attachment != ''){ echo "data-attachment='".json_encode($attachment)."'";} ?>>
        	<h2>轻击这里,上传或选择一张图片</h2>
        	<div class="n4bt-image n4bt-mask n4bt-upload">
        		<div class="n4bt-close"><span class="dashicons dashicons-trash"></span></div>
        	</div>
		...
		?>
		<div id="n4bt-thumbnails" data-options='<?php echo json_encode($this->options); ?>' <?php if($attachment != ''){ echo "data-attachment='".json_encode($attachment)."'";} ?>>
        	<h2>轻击这里,上传一些图片</h2>
        	<div class="n4bt-images n4bt-mask n4bt-upload n4bt-gridly">
	...
	public function save($term_id,$tt_id){
		if(isset($_POST[$this->options['name']]) && isset($this->options['multiple'])){
			...
			$key			= sanitize_key('n4bt_'.$term_id.'_'.$option_name);
			if(!$this->options['multiple']){
...
*/
function get_n4bt_thumbnails($key,$name = ''){
	if($name == ''){
		...
		$value = get_option('n4bt_'.$key.'_'.$name);
	}
...
/*
	返回的内容必然都是 数组
*/
function get_n4bt_thumbnail_urls($key,$name = ''){
	$value = get_n4bt_thumbnails($key,$name);
	$urls = array();
	if(is_array($value)){
		foreach ($value as $id) {
